...
RGBColor yellowGreen(0.51, 0.6, 0.21);
RGBColor blueGreen(0.16, 0.53, 0.45);
...
void build_practical(World *w) {
    for (int i = 0; i < 120; i += SPACING) {
        add_bb_helper(w, orange, Point3D( i + 1, 0, 0),     SIDE, SIDE, HEIGHT);
        add_bb_helper(w, orange, Point3D( 1, 3 * SPACING + i, 0), SIDE, SIDE, HEIGHT);

        add_bb_helper(w, blueGreen, Point3D( i + 1, SPACING, 0), SIDE, SIDE, 2 * HEIGHT);

        add_bb_helper(w, yellowGreen, Point3D( i + 1,  2 * SPACING, 0), SIDE, SIDE, 4 * HEIGHT);
    }

    // add_sphere_helper(w, red, Point3D( -30, 15, 10), 3);
    // add_sphere_helper(w, darkBlue, Point3D( 30, 30, 10), 3);
    ...
    // add_checkerboard(w, mediumGrey, lightGrey, 1);
    add_checkerboard(w, mediumGrey, lightGrey, 1);
}


void build_sphere_triangle_box(World* w) {
    add_sphere_helper() // TODO FILL IN CORRECT VALUES
    add_bb_helper() // TODO FILL IN CORRECT VALUES
    ...
    Triangle* triangle
        = new Triangle(Point3D(), Point3D(), Point3D());  /* TODO FILL IN CORRECT VALUES */
...
void build_sphere_triangle_box_world(World* w, CHOICE choice) {
    ...
    double x = 0.0, y = 0.0, z = 0.0;  /* FILL IN CORRECT VALUES BELOW */

    switch(choice) {
    case A:    // front
        // camera->set_eye(x, y, z);  // NEED DIFFERENT VALUES FOR EACH CASE
        // camera->set_lookat(x, y, z);
        // camera->set_view_distance(distance);
        break;
    case B:    // medium
        // camera->set_eye(x, y, z);
        // camera->set_lookat(x, y, z);
        // camera->set_view_distance(distance);
        break;
    case C:    // overhead
    ...
    lt->set_location(x, y, z);      /* TODO FILL IN CORRECT VALUES */
    lt->set_shadows(true);
    lt->scale_radiance(x);          /* TOOD FILL IN CORRECT VALUE */
    w->add_light(lt);
